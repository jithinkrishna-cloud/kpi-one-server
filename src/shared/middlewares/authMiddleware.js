...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return error(
        res,
        "Authorization token is required (header or cookie)",
        null,
        401,
      );
    }
    ...
    try {
      ...
      if (!userId) {
        return error(
          res,
          "Unauthorized: Token payload is missing User ID",
          null,
          401,
        );
      }
    ...
    if (!employeeData) {
      return error(
        res,
        "Internal Error: Could not retrieve or cache user metadata from ONE CRM",
        null,
        500,
      );
    }
    ...
    return error(
      res,
      err.message || "Unauthorized: Token verification failed",
      null,
      401,
    );
  }
