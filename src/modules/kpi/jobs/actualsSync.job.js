import * as oneApi from "../integrations/oneApi.service.js";
...
export const syncExecutiveActuals = async (executiveId, token, range) => {
    ...
    try {
        ...
        const [leads, deals, quotes, orders, callLogs, messageLogs] = await Promise.all([
        ...
        const metrics = [
            { code: KPI_CODES.LEADS_GENERATED, value: leads?.total || leads?.length || 0 },
            { code: KPI_CODES.DEALS_CREATED, value: deals?.total || deals?.length || 0 },
            { code: KPI_CODES.QUOTES_SUBMITTED, value: quotes?.total || quotes?.length || 0 },
            { code: KPI_CODES.TOUCHPOINT_CALLS, value: callLogs?.total || callLogs?.length || 0 },
            { code: KPI_CODES.TOUCHPOINT_WHATSAPP, value: messageLogs?.total || messageLogs?.length || 0 },
        ];
