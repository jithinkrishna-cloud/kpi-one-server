...
export const createPeriod = async (periodData) => {
    ...
    return await repository.createPeriod(period);
};
...
export const approvePeriod = async (id, approvedBy) => {
    return await repository.withTransaction(async (connection) => {
        ...
        if (period.status === PERIOD_STATUS.ACTIVE) return { message: "Period already active." };
        if (period.status === PERIOD_STATUS.CLOSED) throw new Error("Cannot approve a closed period.");
        if (period.status === PERIOD_STATUS.REJECTED) throw new Error("Rejected periods must be recreated.");

        // 3. Empty Period Check
        const targetCount = await repository.countTargetsByPeriod(id);
        if (targetCount === 0) {
            throw new Error("Cannot approve an empty period. Please set targets first.");
        }

        // 4. Update Status
        await repository.updatePeriodStatus(id, PERIOD_STATUS.ACTIVE, approvedBy);
...
export const rejectPeriod = async (id, rejectedBy, reason) => {
    ...
    return await repository.withTransaction(async (connection) => {
        ...
        if (period.status === PERIOD_STATUS.REJECTED) return { message: "Period already rejected." };
        if (period.status === PERIOD_STATUS.ACTIVE) throw new Error("Cannot reject an active period.");
        if (period.status === PERIOD_STATUS.CLOSED) throw new Error("Cannot reject a closed period.");

        await repository.updatePeriodStatus(id, PERIOD_STATUS.REJECTED, null, reason);
