...
const runTests = async () => {
  ...
  try {
    ...
    const [teamTarget] = await db.query(
      "SELECT * FROM kpi_team_targets WHERE team_id = 'TEAM_RED' AND period_id = ? AND kpi_code = ?",
      [periodId, KPI_CODES.SALES_REVENUE],
    );
    console.log(
      `   ✅ Target Created. Team Auto-Sum: ${teamTarget[0].auto_sum} | Final: ${teamTarget[0].final_value}`,
    );

    // Test 3: Override Stability (Final Value Resolution)
    console.log("\n🔥 Test 3: Override Stability (Final Value Resolution)");
    await targetService.overrideTeamTarget(
      "TEAM_RED",
      periodId,
      KPI_CODES.SALES_REVENUE,
      5000,
    ...
    const [teamTargetUpdated] = await db.query(
      "SELECT * FROM kpi_team_targets WHERE team_id = 'TEAM_RED' AND period_id = ? AND kpi_code = ?",
      [periodId, KPI_CODES.SALES_REVENUE],
    );
    ...
    if (parseFloat(teamTargetUpdated[0].final_value) === 5000) {
      console.log(
        `   ✅ Success: Override Preserved. Auto-Sum: ${teamTargetUpdated[0].auto_sum} | Final: ${teamTargetUpdated[0].final_value}`,
      );
    } else {
      console.log(
        `   ❌ Failure: Override Lost! Final: ${teamTargetUpdated[0].final_value}`,
      );
    }
    ...
    console.log("\n🔥 Test 4: Idempotent Approval");
    await periodService.approvePeriod(periodId, adminUser.id);
    const [p1] = await db.query("SELECT status FROM kpi_periods WHERE id = ?", [
      periodId,
    ]);
    ...
    console.log(
      `   ✅ Success: Second call handled - ${result.message || "OK"}`,
    );
    ...
    try {
      ...
      console.log(
        "   ✅ Success: Edits blocked on rejected period -",
        err.message,
      );
    }
    ...
    const [logs] = await db.query(
      "SELECT * FROM kpi_audit_log WHERE record_id = ? AND entity_type = 'target' ORDER BY id DESC LIMIT 1",
      [5002],
    );
    if (logs.length > 0) {
    ...
    await db.query("DELETE FROM kpi_targets WHERE period_id IN (?, ?)", [
      periodId,
      rejectId,
    ]);
    await db.query("DELETE FROM kpi_team_targets WHERE period_id IN (?, ?)", [
      periodId,
      rejectId,
    ]);
    await db.query("DELETE FROM kpi_periods WHERE id IN (?, ?)", [
      periodId,
      rejectId,
    ]);
    await db.query("DELETE FROM kpi_audit_log WHERE record_id IN (?, ?, ?)", [
      periodId,
      rejectId,
      5002,
    ]);
